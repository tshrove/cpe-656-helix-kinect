...
namespace Iava.Gesture.GestureStuff {
    class UpSwipeSegment1 : IGestureSegment {
            ...
            if (skeleton.Joints[JointID.HandRight].Position.Z < skeleton.Joints[JointID.ShoulderCenter].Position.Z && skeleton.Joints[JointID.HandLeft].Position.Y < skeleton.Joints[JointID.HipCenter].Position.Y) {
                ...
                if (skeleton.Joints[JointID.HandRight].Position.X < skeleton.Joints[JointID.ShoulderRight].Position.X && skeleton.Joints[JointID.HandRight].Position.X > skeleton.Joints[JointID.ShoulderLeft].Position.X) {
                    ...
                    if (skeleton.Joints[JointID.HandRight].Position.Y < skeleton.Joints[JointID.Spine].Position.Y) {
                        Console.WriteLine("Up Swipe Gesture - Segment 1 received.");
                        return GestureResult.Succeed;
                    }

                    // Console.WriteLine("Up Swipe Gesture Segment 1 - Right hand below the chest/gut - UNDETERMINED");
                    return GestureResult.Pause;
                }

                // Console.WriteLine("Up Swipe Gesture Segment 1 - Right hand between shoulders - FAIL");
                return GestureResult.Fail;
            }

            // Console.WriteLine("Up Swipe Gesture Segment 1 - Right hand in front of body with Left hand hanging at the side - FAIL");
            return GestureResult.Fail;
        }
    }
    
    class UpSwipeSegment2 : IGestureSegment {
        public GestureResult CheckGesture(Microsoft.Research.Kinect.Nui.SkeletonData skeleton) {
            // Right hand in front of body with Left hand hanging at the side
            if (skeleton.Joints[JointID.HandRight].Position.Z < skeleton.Joints[JointID.ShoulderCenter].Position.Z && skeleton.Joints[JointID.HandLeft].Position.Y < skeleton.Joints[JointID.HipCenter].Position.Y) {
                // Console.WriteLine("GesturePart 0 - Right hand in front of body - PASS");
                ...
                if (skeleton.Joints[JointID.HandRight].Position.X < skeleton.Joints[JointID.ShoulderRight].Position.X && skeleton.Joints[JointID.HandRight].Position.X > skeleton.Joints[JointID.ShoulderLeft].Position.X) {
                    ...
                    if (skeleton.Joints[JointID.HandRight].Position.Y < skeleton.Joints[JointID.ShoulderCenter].Position.Y && skeleton.Joints[JointID.HandRight].Position.Y > skeleton.Joints[JointID.Spine].Position.Y) {
                        Console.WriteLine("Up Swipe Gesture - Segment 2 received.");
                        return GestureResult.Succeed;
                    }

                    // Console.WriteLine("Up Swipe Gesture Segment 2 - Right hand between the shoulders and chest/gut - UNDETERMINED");
                    return GestureResult.Pause;
                }

                // Console.WriteLine("Up Swipe Gesture Segment 2 - Right hand between shoulders - FAIL");
                return GestureResult.Fail;
            }

            // Console.WriteLine("Up Swipe Gesture Segment 2 - Right hand in front of body with Left hand hanging at the side - FAIL");
            return GestureResult.Fail;
        }
    }

    class UpSwipeSegment3 : IGestureSegment {
        public GestureResult CheckGesture(Microsoft.Research.Kinect.Nui.SkeletonData skeleton) {
            // Right hand in front of body with Left hand hanging at the side
            if (skeleton.Joints[JointID.HandRight].Position.Z < skeleton.Joints[JointID.ShoulderCenter].Position.Z && skeleton.Joints[JointID.HandLeft].Position.Y < skeleton.Joints[JointID.HipCenter].Position.Y) {
                // Console.WriteLine("GesturePart 0 - Right hand in front of body - PASS");
                ...
                if (skeleton.Joints[JointID.HandRight].Position.X < skeleton.Joints[JointID.ShoulderRight].Position.X && skeleton.Joints[JointID.HandRight].Position.X > skeleton.Joints[JointID.ShoulderLeft].Position.X) {
                    ...
                    if (skeleton.Joints[JointID.HandRight].Position.Y > skeleton.Joints[JointID.ShoulderCenter].Position.Y) {
                        Console.WriteLine("Up Swipe Gesture - Segment 3 received.");
                        return GestureResult.Succeed;
                    }

                    // Console.WriteLine("Up Swipe Gesture Segment 3 - Right hand above the shoulders - UNDETERMINED");
                    return GestureResult.Pause;
                }

                // Console.WriteLine("Up Swipe Gesture Segment 3 - Right hand between shoulders - FAIL");
                return GestureResult.Fail;
            }

            // Console.WriteLine("Up Swipe Gesture Segment 3 - Right hand in front of body with Left hand hanging at the side - FAIL");
            return GestureResult.Fail;
        }
    }
}
