            ...
            if (skeleton.Joints[JointID.HandRight].Position.Z < skeleton.Joints[JointID.ShoulderCenter].Position.Z && skeleton.Joints[JointID.HandLeft].Position.Y < skeleton.Joints[JointID.HipCenter].Position.Y) {
                ...
                if (skeleton.Joints[JointID.HandRight].Position.X < skeleton.Joints[JointID.ShoulderRight].Position.X && skeleton.Joints[JointID.HandRight].Position.X > skeleton.Joints[JointID.ShoulderLeft].Position.X) {
                    ...
                    if (skeleton.Joints[JointID.HandRight].Position.Y > skeleton.Joints[JointID.ShoulderCenter].Position.Y) {
                        Console.WriteLine("Down Swipe Gesture - Segment 1 received.");
                        return GestureResult.Succeed;
                    }

                    // Console.WriteLine("Down Swipe Gesture Segment 1 - Right hand above the shoulders - UNDETERMINED");
                    return GestureResult.Pause;
                }

                // Console.WriteLine("Down Swipe Gesture Segment 1 - Right hand between shoulders - FAIL");
                return GestureResult.Fail;
            }

            // Console.WriteLine("Down Swipe Gesture Segment 1 - Right hand in front of body with Left hand hanging at the side - FAIL");
            return GestureResult.Fail;
        }
    }
            ...
            if (skeleton.Joints[JointID.HandRight].Position.Z < skeleton.Joints[JointID.ShoulderCenter].Position.Z && skeleton.Joints[JointID.HandLeft].Position.Y < skeleton.Joints[JointID.HipCenter].Position.Y) {
                ...
                if (skeleton.Joints[JointID.HandRight].Position.X < skeleton.Joints[JointID.ShoulderRight].Position.X && skeleton.Joints[JointID.HandRight].Position.X > skeleton.Joints[JointID.ShoulderLeft].Position.X) {
                    ...
                    if (skeleton.Joints[JointID.HandRight].Position.Y < skeleton.Joints[JointID.ShoulderCenter].Position.Y && skeleton.Joints[JointID.HandRight].Position.Y > skeleton.Joints[JointID.Spine].Position.Y) {
                        Console.WriteLine("Down Swipe Gesture - Segment 2 received.");
                        return GestureResult.Succeed;
                    }

                    // Console.WriteLine("Down Swipe Gesture Segment 2 - Right hand between the shoulders and chest/gut - UNDETERMINED");
                    return GestureResult.Pause;
                }

                // Console.WriteLine("Down Swipe Gesture Segment 2 - Right hand between shoulders - FAIL");
                return GestureResult.Fail;
            }

            // Console.WriteLine("Down Swipe Gesture Segment 2 - Right hand in front of body with Left hand hanging at the side - FAIL");
            return GestureResult.Fail;
        }
    }
            ...
            if (skeleton.Joints[JointID.HandRight].Position.Z < skeleton.Joints[JointID.ShoulderCenter].Position.Z && skeleton.Joints[JointID.HandLeft].Position.Y < skeleton.Joints[JointID.HipCenter].Position.Y) {
                ...
                if (skeleton.Joints[JointID.HandRight].Position.X < skeleton.Joints[JointID.ShoulderRight].Position.X && skeleton.Joints[JointID.HandRight].Position.X > skeleton.Joints[JointID.ShoulderLeft].Position.X) {
                    ...
                    if (skeleton.Joints[JointID.HandRight].Position.Y < skeleton.Joints[JointID.Spine].Position.Y) {
                        Console.WriteLine("Down Swipe Gesture - Segment 3 received.");
                        return GestureResult.Succeed;
                    }

                    // Console.WriteLine("Down Swipe Gesture Segment 3 - Right hand below the chest/gut - UNDETERMINED");
                    return GestureResult.Pause;
                }

                // Console.WriteLine("Down Swipe Gesture Segment 3 - Right hand between shoulders - FAIL");
                return GestureResult.Fail;
            }

            // Console.WriteLine("Down Swipe Gesture Segment 3 - Right hand in front of body with Left hand hanging at the side - FAIL");
            return GestureResult.Fail;
        }
    }
}
